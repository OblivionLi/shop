...
export const createParentCat = (parent_category_name) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await Axios.post(`/api/parentCat`, { parent_category_name }, config);
...
export const editParentCat = (parentId, parent_category_name) => async (
    dispatch,
    getState
) => {
    try {
